...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        boolGoal = False
        visitcorner = state[1]
        if visitcorner == [True,True,True,True]:
            boolGoal = True
        return boolGoal
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not isWall:
                ...
                if indcorner < 4:
                    newvisitcorner[indcorner] = True
                stateNew = ((newx,newy),newvisitcorner)
                cost = self.costFn(newx,newy)
...
def cornersHeuristic(state, problem):
    ...
    for x in range(4):
        if not cornersVisited[x]:
            dist = abs(pos_cur[0] - corners[x][0]) + abs(pos_cur[1] - corners[x][1])
            if dist > mostdist: 
                mostdist = dist
